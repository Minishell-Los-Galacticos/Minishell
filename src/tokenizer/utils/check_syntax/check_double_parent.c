...
/*   Updated: 2025/09/16 17:37:42 by davdiaz-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../../../inc/minishell.h"

static int put_space_between(t_shell *d, char **str, char **tmp, char **space)
{
...
static void ignore_till_paren_close(t_prompt *prompt, t_token *tokens, int *i)
{
...
int check_double_parent(t_shell *data, t_token *tokens, t_prompt *p)
{
	...
	while (i < p->n_tokens)
	{
		if (tokens[i].type == PAREN_OPEN && tokens[i + 1].type == PAREN_OPEN)
		{
			i += 2;
			start = i;
			ignore_till_paren_close(p, tokens, &i);
